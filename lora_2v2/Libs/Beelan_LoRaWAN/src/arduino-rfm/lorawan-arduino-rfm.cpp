...
void LoRaWANClass::sendUplink(char *data, unsigned int len, bool confirm, int mport) {
    if (currentChannel == MULTI) {
        randomChannel();
    }
    LoRa_Settings.Confirm = confirm;
    // Validate and adjust mport
    mport = (mport == 0 || mport > 223) ? 1 : mport;
    LoRa_Settings.Mport = mport;
    // Set new command for RFM
    RFM_Command_Status = NEW_RFM_COMMAND;
    // Set counter and copy data to Tx buffer
